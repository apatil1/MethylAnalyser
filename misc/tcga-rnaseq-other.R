...
#LUSC
path_folder <-"D:/ONC-Analysis/users/ash/michelle/lusc_rnaseq/RNASeq/UNC__IlluminaHiSeq_RNASeq/Level_3/"
...
data <- rnaseqv2_read(path_folder)

#LUAD
path_folder <-"D:/ONC-Analysis/users/ash/michelle/luad_rnaseq/RNASeq/UNC__IlluminaHiSeq_RNASeq/Level_3/"
...
rnaseqv2_read <- function(path_folder) {
  ...
  for (i in 1:length(file_names)) {
    ...
    if (i==1) {
      gene <- strsplit(as.character(read$gene[1]),"[|]")[[1]][1]
      for (j in 2:length(read$gene)) {
        gene <- append(gene, strsplit(as.character(read$gene[j]),"[|]")[[1]][1])
      }
      
      data <- cbind.data.frame(gene, read[,4])
      ex <- substr(file_names[i], 14, 41)
      
      colnames <- c("gene",ex)
    }
    else {
      data <- cbind.data.frame(data, read[,4])
      ex <- substr(file_names[i], 14, 41)
      colnames <- append(colnames, ex )
    }
...
#lusc
heat_tcga_scale(topvar, plotName="rnaseq_scaletopvar_lusc.jpg", title="Heatmap LUSC")
heat_tcga(topvar, plotName="rnaseq_topvar_lusc.jpg", title="Heatmap LUSC")

#luad
heat_tcga_scale(topvar, plotName="rnaseq_scaletopvar_luad.jpg", title="Heatmap LUAD")
heat_tcga(topvar, plotName="rnaseq_topvar_luad.jpg", title="Heatmap LUAD")
...
#luaad
heat_tcga_scale(tcga_up, plotName="scale-15m-rnaseq-heatmap_tcga_up_LUAD.jpg", title="Heatmap RNAseq tcga up LUAD")
heat_tcga_scale(tcga_down, plotName="scale-15m-rnaseq-heatmap_tcga_down_LUAD.jpg", title="Heatmap RNAseq tcga down LUAD")
...
#lusc
heat_tcga_scale(tcga_up, plotName="scale-15m-rnaseq-heatmap_tcga_up_LUSC.jpg", title="Heatmap RNAseq tcga up LUSC")
heat_tcga_scale(tcga_down, plotName="scale-15m-rnaseq-heatmap_tcga_down_LUSC.jpg", title="Heatmap RNAseq tcga down LUSC")
